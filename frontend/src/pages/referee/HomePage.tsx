...
export const getWinnerName = (winner?: Maybe<Team> | undefined) => {
  let name: string = "";
  if (winner?.composition === CompositionEnum.Single) {
    name = winner.name || "";
  } else if (winner?.composition === CompositionEnum.Double) {
    if (winner.players?.length) {
      name = winner.players.map((p) => playerShort(p)).join(" / ");
    } else {
      name = "";
    }
  }
