...
export default function PlayerTournamentDetail() {
  ...
  const { id } = useParams();
  const [{ data: tournamentData }, refetchTournament] = useTournamentQuery({
    variables: {
      id: id || "",
    },
  });
  ...
  const [{ data: joinableTeamsData }, refetchJoinableTeams] =
    useJoinableTeamsQuery({
      variables: {
  ...
  const joinableTeams = joinableTeamsData
    ...
    : [];
  const [{ data: joinedTournamentsData }, refetchJoinedTournaments] =
    useJoinedTournamentsQuery();
  const joinedTournaments = joinedTournamentsData
    ? (joinedTournamentsData.me as Player).joinedTournaments
    : [];
  const isRegistered = joinedTournaments?.some((t) => t.id === tournament?.id);
  ...
  const handleRegister = async () => {
    ...
    if (res.data?.joinTournament) {
      ...
      refetchJoinedTournaments({
        requestPolicy: "network-only",
      });
      refetchJoinableTeams({
        requestPolicy: "network-only",
      });
    }
  };
